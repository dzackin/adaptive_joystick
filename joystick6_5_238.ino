...
int scrollDirection = 1;           // 1 means normal scrolling, -1 means reverse scrolling
  ...
  if (switchState == HIGH) {
    ...
    if (digitalRead(mouseLeftButton) == HIGH) {
      ...
      if (digitalRead(mouseRightButton) == HIGH) {
        Serial.print("BOTH BUTTONS PUSHED axis=");
        Serial.println(readAxis(yAxis));
        if (readAxis(yAxis) < 0 && range < maxRange) {
          range++;
          Serial.println("INCREASING RANGE");
        } else if (readAxis(yAxis) > 0 && range > minRange) {
          range--;
          Serial.println("DECREASING RANGE");
        } else if (digitalRead(mouseUpScroll) == HIGH) {
          scrollDirection *= -1;
          Serial.print("SCROLL DIRECTION IS NOW ");
          Serial.println(scrollDirection);
          delay(2000);
        }
    ...
    if (digitalRead(mouseUpScroll) == HIGH) {
      ...
      if (lastScrollUpState == LOW) {
        ...
        for (int x = 0; x < scrollLinesPerPress; x++) {
          Mouse.move(0, 0, -1 * scrollDirection);
        }
